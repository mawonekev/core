...
BASE_QUERYSET = TourOperator.objects.filter(is_active=True, deleted_at__isnull=True, is_approved=True).prefetch_related('attractions')
...
@extend_schema(
    ...
    description=(
        ...
        '**POST** — Submit a new tour operator. **Requires staff/admin privileges** (prevents scammers).\n\n'
        '**curl GET example:**\n'
        '```bash\n'
    ...
    responses={
        ...
        403: OpenApiResponse(description='Only admin/staff can create tour operators (anti-scam protection).'),
    },
)
@api_view(['GET', 'POST'])
...
@extend_schema(
    ...
    description=(
        ...
        '- **PUT/PATCH** — Update. **Admin/staff only** (prevents unauthorized modifications).\n'
        '- **DELETE** — Remove operator. **Admin/staff only**.\n\n'
        '**curl GET example:**\n'
        '```bash\n'
        'curl https://xenohuru.cleven.is-a.dev/api/v1/operators/serengeti-adventures/\n'
        '```'
    ),
    responses={
        ...
        403: OpenApiResponse(description='Only admin/staff can modify tour operators.'),
...
@extend_schema(
    ...
    description=(
        ...
        '**curl example:**\n'
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/operators/by_attraction/?attraction=mount-kilimanjaro"\n'
        '```'
    ),
    parameters=[
        OpenApiParameter(
