...
from app.core.mixins import ImageMixin, PriceMixin, SEOMixin
...
class Attraction(BaseModel, SEOMixin, ImageMixin, PriceMixin, models.Model):
    DIFFICULTY_CHOICES = [
        ('easy', 'Easy'),
        ('moderate', 'Moderate'),
    ...
    # Location data - keep specific fields for attractions (altitude, etc.)
    latitude = models.DecimalField(max_digits=9, decimal_places=6)
    longitude = models.DecimalField(max_digits=9, decimal_places=6)
    altitude = models.IntegerField(help_text='Altitude in meters', null=True, blank=True)

    # Tags
    tags = models.ManyToManyField('core.Tag', blank=True, related_name='attractions')
    ...
    entrance_fee = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text='Fee in USD (DEPRECATED: use price_from)')
    ...
    # Metadata
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='created_attractions')
    updated_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='updated_attractions')
