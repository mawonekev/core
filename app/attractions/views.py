...
from .models import Attraction, AttractionBoundary, Citation, EndemicSpecies, NearestTransport
...
BASE_QUERYSET = Attraction.objects.filter(is_active=True, deleted_at__isnull=True, is_approved=True).select_related('region', 'created_by').prefetch_related('images', 'tips', 'endemic_species')
...
@extend_schema(
    ...
    description=(
        ...
        '**curl GET example:**\n'
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/attractions/?search=kilimanjaro"\n'
        '```\n\n'
        '**curl POST example:**\n'
        '```bash\n'
        'curl -X POST https://xenohuru.cleven.is-a.dev/api/v1/attractions/ \\\n'
...
def attraction_list_create(request):
    if request.method == 'GET':
        ...
        return paginate_queryset(attractions, request, AttractionListSerializer)
...
@extend_schema(
    ...
    description=(
        ...
        '**curl GET example:**\n'
        '```bash\n'
        'curl https://xenohuru.cleven.is-a.dev/api/v1/attractions/mount-kilimanjaro/\n'
        '```\n\n'
        '**curl PATCH example:**\n'
        '```bash\n'
        'curl -X PATCH https://xenohuru.cleven.is-a.dev/api/v1/attractions/mount-kilimanjaro/ \\\n'
        '  -H "Authorization: Bearer <access_token>" \\\n'
        '  -H "Content-Type: application/json" \\\n'
...
@extend_schema(
    ...
    description=(
        ...
        '**curl example:**\n'
        '```bash\n'
        'curl https://xenohuru.cleven.is-a.dev/api/v1/attractions/featured/\n'
        '```'
    ),
    responses={
...
@extend_schema(
    ...
    description=(
        ...
        '**curl example:**\n'
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/attractions/by_category/?category=national_park"\n'
        '```'
    ),
    parameters=[
        OpenApiParameter(
...
@extend_schema(
    ...
    description=(
        ...
        '**curl example:**\n'
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/attractions/by_region/?region=arusha"\n'
        '```'
    ),
    parameters=[
        OpenApiParameter(
...
@extend_schema(
    ...
    description=(
        ...
        '**curl example:**\n'
        '```bash\n'
        'curl https://xenohuru.cleven.is-a.dev/api/v1/attractions/serengeti/endemic-species/\n'
        '```'
    ),
    responses={
