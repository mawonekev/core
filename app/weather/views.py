...
from .serializers import CurrentWeatherSerializer, SeasonalWeatherPatternSerializer, WeatherCacheSerializer
...
@extend_schema(
    ...
    description=(
        ...
        '**curl example:**\n'
        '```bash\n'
        'curl https://xenohuru.cleven.is-a.dev/api/v1/weather/\n'
        '```'
    ),
    responses={
        200: OpenApiResponse(
...
@extend_schema(
    ...
    description=(
        ...
        '**curl example:**\n'
        '```bash\n'
        'curl https://xenohuru.cleven.is-a.dev/api/v1/weather/1/\n'
        '```'
    ),
    responses={
        200: OpenApiResponse(
...
@extend_schema(
    ...
    description=(
        ...
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/weather/current/?lat=-3.0674&lon=37.3556"\n'
        '```\n\n'
        '**curl example (by attraction slug):**\n'
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/weather/current/?attraction=mount-kilimanjaro"\n'
        '```'
    ),
    parameters=[
...
def current_weather(request):
    ...
    # Accept both 'lon' and 'lng' for longitude
    lon = request.query_params.get('lon') or request.query_params.get('lng')
    attraction_slug = request.query_params.get('attraction') or request.query_params.get('slug')
...
@extend_schema(
    ...
    description=(
        ...
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/weather/forecast/?attraction=mount-kilimanjaro&days=7"\n'
        '```\n\n'
        '**curl example (3-day forecast by coordinates):**\n'
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/weather/forecast/?lat=-3.0674&lon=37.3556&days=3"\n'
        '```'
    ),
    parameters=[
...
@extend_schema(
    ...
    description=(
        ...
        '**curl example:**\n'
        '```bash\n'
        'curl "https://xenohuru.cleven.is-a.dev/api/v1/weather/seasonal/?attraction=mount-kilimanjaro"\n'
        '```'
    ),
    parameters=[
        OpenApiParameter(
