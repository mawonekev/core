...
@extend_schema(
    ...
    description=(
        ...
        '**curl GET example:**\n'
        '```bash\n'
        'curl https://xenohuru.cleven.is-a.dev/api/v1/regions/\n'
        '```\n\n'
        '**curl POST example:**\n'
        '```bash\n'
        'curl -X POST https://xenohuru.cleven.is-a.dev/api/v1/regions/ \\\n'
...
def region_list_create(request):
    if request.method == 'GET':
        regions = Region.objects.filter(deleted_at__isnull=True, is_approved=True)

        # Use pagination helper
        return paginate_queryset(regions, request, RegionSerializer)
...
@extend_schema(
    ...
    description=(
        ...
        '**curl GET example:**\n'
        '```bash\n'
        'curl https://xenohuru.cleven.is-a.dev/api/v1/regions/arusha/\n'
        '```\n\n'
        '**curl PATCH example:**\n'
        '```bash\n'
        'curl -X PATCH https://xenohuru.cleven.is-a.dev/api/v1/regions/arusha/ \\\n'
        '  -H "Authorization: Bearer <access_token>" \\\n'
        '  -H "Content-Type: application/json" \\\n'
